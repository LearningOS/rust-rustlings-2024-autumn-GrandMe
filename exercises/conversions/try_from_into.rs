...
// 辅助函数，用于检查单个i16值是否在有效范围内  
fn check_value(value: i16) -> Result<(), IntoColorError> {  
...
// 辅助函数，用于从i16切片创建Color  
fn create_color_from_slice(slice: &[i16]) -> Result<Color, IntoColorError> {  
    if slice.len() != 3 {  
        return Err(IntoColorError::BadLen);  
    }  
    for &value in slice {  
...
impl TryFrom<(i16, i16, i16)> for Color {  
    type Error = IntoColorError;  
    fn try_from(tuple: (i16, i16, i16)) -> Result<Self, Self::Error> {  
        check_value(tuple.0).and_then(|_| check_value(tuple.1))  
            .and_then(|_| check_value(tuple.2))  
            .map(|_| Color {  
                red: tuple.0 as u8,  
                green: tuple.1 as u8,  
                blue: tuple.2 as u8,  
